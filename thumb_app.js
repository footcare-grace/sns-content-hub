...
${stat?`- 統計の帯：メインタイトルの直下に、濃紺（#1e2a4a）で塗りつぶした横長の帯（バー）を1本配置すること。帯の中に「${stat}」という文言を白文字・太字で入れ、特に数字部分だけをオレンジ（#e8681a）にして目立たせること。帯は左右に画面幅の6%の余白を残し、高さはメインタイトル1行分程度。動きのある印象にするため、帯は完全な水平ではなく、わずかに右肩上がりの傾き（3〜5度程度）をつけてもよい`:""}
- サブタイトル「${sub}」：オレンジ（#e8681a）、太字、文字の高さはメインタイトルの約48%。${stat?"統計の帯の下、":"メインタイトルの下、"}余白（画面高さの3%程度）を空けて配置
...
- イラストは1つだけにすること。円形の吹き出し・ワイプで人物写真や別カットを追加で挿入しないこと（メインイラスト以外の2つ目の画像要素は一切追加しない）
