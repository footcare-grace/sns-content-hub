...
function gakyoFor(v){
  ...
  const[kind,id]=v.split(":");
  if(kind!=="t")return null;
  return (typeof GAKYO!=="undefined")?GAKYO[id]:null;
}
...
${theme}`);
  ...
  const gakyo=gakyoFor(kv);
  if(gakyo)parts.push("",`■ このテーマの「佳境」（セミナー佳境分析・佳境比較分析より。無料/有料の線引きの実例）
- 無料Noteに入れるべき佳境（Why側・実際に会場が静まった/驚かれたフレーズ）：
  ${gakyo.free}
- 有料Noteに残すべき核心（How側・現場でしか知れない具体的な手順）：
  ${gakyo.paid}
- 章立てを設計する際、無料部分は上記の「無料佳境」を中心に構成し、有料部分は「有料核心」に直結する内容にすること`);

  if(typeof NOTE_10RULES!=="undefined")parts.push("",NOTE_10RULES);
  if(typeof COMPETITIVE_DIFF!=="undefined")parts.push("",COMPETITIVE_DIFF);
  if(typeof PASONA_EXAMPLE!=="undefined")parts.push("",PASONA_EXAMPLE);
...
- 境界＝「答えの直前」で切ること${gakyo?"（上記の「佳境」データを踏まえて具体的に線引きすること）":""}
- どの章とどの章の間で切るべきか、理由付きで提案すること
- 無料部分の最後は「続きを読みたくなる引き」で終わる設計にすること

■ 序章「このNoteを読むとわかること」の設計（必ず本文の冒頭・第1章の前に置くこと）
- 目的は2つ：①読者に安心感を与える　②有料ゾーンへ違和感なく進んでもらう（離脱防止）
- 【安心感の作り方】
  - 読者の悩み（ペルソナの「切実な悩み」）を具体的に言い当て、「自分のことだ」「分かってもらえている」と感じさせること
  - 「あなたのせいではない」「ちゃんと理由がある」という、原因の所在を明確にするニュアンスを含めること
- 【書いてよい内容・書いてはいけない内容】
  - 書いてよい：読了後に得られる「気づき・心理的な変化」（ペルソナの「欲しい未来」を土台にすること）
  - 書いてはいけない：具体的な手順・答えそのもの（Howの中身に触れない）。医療効果を断定する表現
- 【離脱防止の作り方】
  - 箇条書き3行程度で「わかること」を提示し、①②は無料部分だけで完結する価値（読んで良かったと思える内容）、③は有料部分に自然につながる一文にすること
  - 「答えが分かれば怖くない」というトーンにし、有料部分を「壁」ではなく「次の一歩」として感じさせること
  - 煽り・不安を過度に強調する表現は使わないこと（安心感が目的のため）`);
  if(!isSplit&&!isFree)parts.push("",`■ 全文有料の設計
- 購入前に読める「記事説明文（リード文）」の構成案も併せて提案すること`);

  parts.push("",YAKKIHO_RULES,"",`【出力形式】
1. 記事タイトル案（3つ）
2. 序章「このNoteを読むとわかること」の文案（安心感・離脱防止の両方を満たすもの）
3. 章立て（各章のタイトル＋その章で扱う内容の要点2〜3行）
${isSplit&&!isFree?"4. 無料/有料の境界線の提案（理由付き）":""}
の順で出力してください。`);
...
- 導入は読者の悩みへの共感から始めること
- 章立てに「序章（このNoteを読むとわかること）」が含まれている場合は、本文の一番最初にそのまま執筆すること。安心感（悩みを言い当てる・原因の所在を示す）と、有料ゾーンへの自然な導線（煽らず「次の一歩」と感じさせる）の両方を満たすこと。具体的な手順・答えそのものはここでは書かないこと
- 知識ベースの比喩・エピソード（例：トマトケチャップ現象、円柱形理論など）は積極的に活用し、原文の意味を保つこと
- 1つの見出しあたり400〜600字を目安にすること
- 箇条書き・小見出しを適度に使い、スマホでも読みやすいリズムにすること
- 各章の終わりに、次の章へ読み進めたくなる一文を入れること`,"",(typeof NOTE_10RULES!=="undefined"?NOTE_10RULES:""),"",YAKKIHO_RULES,"",`【出力形式】
