...
- CTAは「フォロー」を促す軽いトーンにすること（セミナー詳細への誘導は入れない）`;
  }else if(goal.includes("興味喚起")){
    goalRule=`- ゴール：フォロー中の人にセミナーへの興味を持たせること
- CTAは「プロフィールのリンクから詳細を見る」等、次の行動への軽い誘導にすること`;
  }else{
    goalRule=`- ゴール：セミナー申込への後押し（既存フォロワー向け）
- CTAは「2日間で何が変わるか」の具体性を出し、申込への直接的な誘導にすること`;
  }
...
- フックは8つの型（警告型・逆説型・盲点型・驚き型・数字型・体験談型・問いかけ型・常識破壊型）の中から、内容に最も合うものを選び、型名を明記すること
${goalRule}
...
【出力形式】
まず0〜2秒のフックを5パターン（型を明記）提示し、その中から最も強いと思うものを1つ選んだ上で、選定理由とともに続き（2〜22秒・22〜30秒）まで含めた完成台本を1本、以下の形式で出力してください。
...
3. 説明から入っていないか（状況説明ではなく、驚き・疑問・断言から始まっているか）
4. 型が明確か（警告型・逆説型・盲点型・驚き型・数字型・体験談型・問いかけ型・常識破壊型のいずれかに当てはまるか）
5. 具体性があるか（誰にでも言えるような一般論になっていないか）

■ 弱いと判定した場合
- なぜ弱いのかを一言で言語化すること
- 同じ核心メッセージを保ったまま、型を変えた改善案を3パターン提示すること
