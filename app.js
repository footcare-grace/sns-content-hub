...
const YAKKIHO_RULES=`■ 薬機法・広告表現の制約（必ず守ること。ただし断言できる部分とできない部分を明確に区別すること）

【断言してよい（曖昧にしない・言い切ること）】
- 知識ベースにある身体のメカニズム・構造・統計データ・研究結果は、そのまま断定して書くこと
  例：「膝には体重の6〜7倍の負荷がかかります」「足底筋膜炎の原因は◯◯です」のように言い切ってよい
- 「〜だと言われています」のような伝聞調にせず、事実として直接的に言い切ること
- 曖昧な表現（〜かもしれません、〜の可能性があります等）で事実をぼかさないこと

【断言してはいけない（この部分だけ薬機法に配慮すること）】
- 「治る」「治す」「治療」「改善する」など、商品・インソール等を使った結果としての医療効果を断定する表現は使わないこと
- 「痛みが消える」「必ず良くなる」「誰でも治る」「◯◯%改善」等の効果保証・数値保証は使わないこと
- 体験談・お客様の声を紹介するときは、個人差がある前提であることが伝わる表現にすること
- 医療行為・診断と誤解される表現（診断します・処方します等）は使わないこと

【判断基準】
「これは身体の仕組み・事実の説明か？」→ 断言してよい
「これは特定の商品・方法を使った後の効果の話か？」→ 薬機法の配慮が必要`;

const TONMANA_DEFAULT="専門家として信頼感を出す（押しつけがましくない）。難しい言葉は使わず、中学生でもわかる言葉で。共感から入り、解決策を示す構成。";

const OUTPUT_TAG_FORMAT=`【投稿NN｜対象層｜テーマ｜断言の型｜エンゲージメント目標】

---投稿NN（1本目）---

（断言文のみ・100文字以内・驚き・逆説・反常識で引きつける）

---投稿NN（2本目）---

その理由は、〇〇です。

（理由・根拠・体験談を200文字以内で展開する）

[断言の型：〇〇型]
[参照：knowledge/ファイル名（該当箇所）]
[推奨投稿時間帯：夕（17:00〜21:00）]
[エンゲージメント：保存 / 拡散・シェア / コメント]
[理由：このテーマ・型がなぜ刺さるかを1〜2行で説明]`;

const DONT_WRITE_LIST=`■ 書かないことリスト（発信範囲・必ず守ること）
- 足・インソールに直結しない疾患（頸椎・肩関節など足との関連が薄いもの）は扱わないこと
- 食事・メンタル・睡眠など生活習慣全般には踏み込まないこと
- インソール以外の物販・器具の紹介はしないこと
...
/* ================= 運用メモ（勝ちパターン・負けパターン） ================= */
function fillLearningsForm(){
  $("#ax-learnings").value=learnings;
  renderLearningsStatus();
}
function renderLearningsStatus(){
  const el=$("#learnings-status");
  if(!el)return;
  el.textContent=learnings.trim()
    ?"✓ この内容が、企画担当・編集担当のプロンプトに自動で組み込まれています"
    :"まだ運用メモがありません。リサーチ担当での分析結果を貼って保存すると、次回から自動反映されます";
}
$("#btn-save-learnings").addEventListener("click",()=>{
  learnings=$("#ax-learnings").value.trim();
  saveLearnings();
  const f=$("#learnings-saved");
  f.classList.add("show");
  setTimeout(()=>f.classList.remove("show"),2000);
  renderLearningsStatus();
});
function learningsBlock(){
  ...
  return `\n■ 過去の分析からの学び（勝ちパターン・避けるべきパターン。必ず活かすこと）\n${learnings}\n`;
}
...
function getPersonaBlock(){
  ...
  if(!p) return null;
  return `■ このテーマの具体的なペルソナ（note_reference/テーマ別ペルソナ設計・一般向けペルソナ設計より。汎用の軸ペルソナより優先すること）
- 一般読者：${p.general.name}。${p.general.situation}
  切実な悩み：${p.general.pain}
  欲しい未来：${p.general.future}（※Howの手順は求めていない。気づき・希望に留めること）
- セラピスト：${p.therapist.name}。${p.therapist.situation}
...
/* ================= バズ投稿ストック ================= */
$("#btn-add-buzz").addEventListener("click",()=>{
  const text=$("#bz-text").value.trim();
  if(!text){alert("投稿本文を入力してください");return;}
  buzzStock.unshift({
    text,
    author:$("#bz-author").value.trim(),
    reason:$("#bz-reason").value.trim(),
    date:new Date().toLocaleDateString("ja-JP")
  });
  saveBuzz();
  $("#bz-text").value="";$("#bz-author").value="";$("#bz-reason").value="";
  renderBuzz();
});
function renderBuzz(){
  const tb=$("#buzz-table tbody");tb.innerHTML="";
  $("#buzz-count").textContent=buzzStock.length;
  if(!buzzStock.length){tb.innerHTML=`<tr><td colspan="5" class="empty-row">まだストックがありません。気になった投稿を見つけたら上のフォームから保存してください</td></tr>`;return;}
  buzzStock.forEach((b,i)=>{
    const tr=document.createElement("tr");
    const shortText=b.text.length>40?b.text.slice(0,40)+"…":b.text;
    tr.innerHTML=`<td title="${esc(b.text)}">${esc(shortText)}</td><td>${esc(b.author)||"—"}</td><td>${esc(b.reason)||"—"}</td><td>${esc(b.date)}</td>
      <td><button class="del-x" data-i="${i}" aria-label="削除">✕</button></td>`;
    tb.appendChild(tr);
  });
  tb.querySelectorAll(".del-x").forEach(btn=>btn.addEventListener("click",()=>{
    const i=Number(btn.dataset.i);
    buzzStock.splice(i,1);saveBuzz();renderBuzz();
  }));
}
$("#btn-send-buzz").addEventListener("click",()=>{
  if(!buzzStock.length){alert("ストックが空です。先に投稿を保存してください");return;}
  const combined=buzzStock.map(b=>b.text).join("\n---\n");
  $$(".role").forEach(x=>x.classList.remove("on"));
  $$(".pane").forEach(x=>x.classList.remove("on"));
  ...
  renderAxisBanners();
  $$("#rs-mode button").forEach(x=>x.classList.remove("on"));
  $('#rs-mode button[data-v="buzz"]').classList.add("on");
  rsMode="buzz";
  $("#rs-label").textContent=RS_META.buzz.label;
  $("#rs-hint").textContent=RS_META.buzz.hint;
...
$("#btn-export").addEventListener("click",()=>{
  const data={axis,tasks,leads,insightLog,buzzStock,learnings,exported:new Date().toISOString()};
...
$("#import-file").addEventListener("change",e=>{
  ...
  r.onload=()=>{
    try{
      ...
      if(Array.isArray(d.buzzStock))buzzStock=d.buzzStock;
      if(typeof d.learnings==="string")learnings=d.learnings;
      saveAxis();saveTasks();saveLeads();saveInsights();saveBuzz();saveLearnings();
      fillAxisForm();fillLearningsForm();renderTasks();renderLeads();renderAxisBanners();renderInsights();renderBuzz();
      alert("復元しました");
