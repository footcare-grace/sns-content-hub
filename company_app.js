...
■ フックの文言の作り方（最重要・厳守）
- 見出しの文言は、上記の情報に書かれている単語・フレーズを直接使って作ること
- 「その不調、原因は足元です」のような、商品によらず使い回せる汎用フレーズは絶対に使わないこと。必ず${productName}特有の悩み・キーワードに言及すること
${realTagline?`- HPの実際のキャッチコピー「${realTagline}」を、そのまま使うか、そのトーン・言葉選びを土台にして見出しを作ること`:""}
${hookTone==="posi"?`- トーンは「ポジ・宣言スローガン型」：問題を言い当てるのではなく、力強く解決を宣言する言い切り文にすること
  例のイメージ（そのまま使わず、商品の悩みワードに置き換えること）：「◯◯撲滅」「もう◯◯とはサヨナラ」のような、短く強いスローガン
- インパクトのある大きな文字で、勝利宣言・決意表明のような力強さを出すこと`
:`- トーンは「ネガ・問題提起型」：具体的な悩みの1つを名指しして、原因や課題を言い切ること
