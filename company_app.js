...
const SLIDE_STRUCTURES={
  1:[{role:"単体",desc:"商品の魅力を1枚に凝縮した紹介画像"}],
  3:[
    {role:"1枚目：フック",desc:"悩み・問題提起を投げかける導入スライド（商品はまだ出さない）"},
    {role:"2枚目：解決",desc:"その悩みに対する解決策として商品を紹介するスライド"},
    {role:"3枚目：商品詳細",desc:"商品の特徴・スペックを図解するスライド"}
  ],
  4:[
    {role:"1枚目：悩み",desc:"対象読者の悩み・共感を引き出す導入スライド（商品はまだ出さない）"},
    {role:"2枚目：原因",desc:"その悩みが起きる原因を説明するスライド（図解要素があるとよい）"},
    {role:"3枚目：解決策",desc:"商品がその原因にどうアプローチするかを示すスライド"},
    {role:"4枚目：商品",desc:"商品写真を主役にした、購入・相談を後押しする最終スライド"}
  ],
  5:[
    {role:"1枚目：悩み",desc:"対象読者の悩み・共感を引き出す導入スライド（商品はまだ出さない）"},
    {role:"2枚目：原因",desc:"その悩みが起きる原因を説明するスライド（図解要素があるとよい）"},
    {role:"3枚目：解決策",desc:"商品がその原因にどうアプローチするかを示すスライド"},
    {role:"4枚目：商品",desc:"商品写真を主役にした、機能訴求スライド"},
    {role:"5枚目：信頼",desc:"お客様の声・保証制度・権威性など、後押しとなる要素を見せる最終スライド"}
  ]
};
...
$("#btn-gen-post").addEventListener("click",()=>{
  ...
  structure.forEach((slide,i)=>{
    const needsProduct=slide.role.includes("商品")||slide.role.includes("解決策")&&structure.length<=3;
    const prompt=`【${slideCount}枚シリーズの${i+1}枚目／全${structure.length}枚】
${needsProduct||slideCount==1?"この文章と一緒に、HPの商品写真を添付してください。":"このスライドは商品写真の添付は不要です（悩み・原因の説明イラストのため）。"}
...
${needsProduct||slideCount==1?"- 商品の特徴を示す短いテキストラベルを2〜3個配置し、矢印・引き出し線で示すこと\n- 商品写真の形状・色は改変しないこと（実物と異なる見た目にしない）":"- 商品写真は使わず、悩み・原因が伝わるシンプルなイラスト・アイコン・図解で構成すること"}
- 文字は日本語で、読みやすく大きめに
...
各プロンプトの中に「写真を添付してください」と書かれているスライドについてのみ、フォルダ内の写真から一番適したものを選び、ファイル名と理由を教えてください。「写真の添付は不要です」と書かれているスライドは、写真選定も不要です。
